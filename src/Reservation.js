...
const Reservation = () => {
  ...
  const fetchReservations = async (page = 1, searchQuery = '') => {
    setLoading(true);
    try {
      const response = await axios.get('http://98.85.246.54:8132/api/reserve/reservations', {
        params: {
          page,
          limit: itemsPerPage,
          search: searchQuery,
  ...
  const handleDeleteReservation = async (id) => {
    try {
      await axios.delete(`http://98.85.246.54:8132/api/reserve/reservation/${id}`); // Adjust the API URL
  ...
  const handleReservationById = async (reservation) => {
    try {
      const response = await axios.get(
        `http://98.85.246.54:8132/api/reserve/reservation/${reservation._id}`
      );
  ...
  const getSeasonAndDays = async (days, pickdate, dropdate) => {
    try {
      const response = await axios.get("http://98.85.246.54:8132/api/newVehicle/vehicleData", {
        params: {
          days,
          pickdate,
          dropdate,
  ...
  const handleUpdateReservation = async (vehicleId, totalPrice) => {
    ...
    try {
      const response = await axios.put(
        `http://98.85.246.54:8132/api/reserve/reservation/${reservationId}`,
        {
          vehicleId,
  ...
  const handleAcceptReservation = async (id) => {
    try {
      const response = await axios.put(`http://98.85.246.54:8132/api/reserve/reservation/${id}/accept`);
  ...
  const handleAddReservation = async () => {
    ...
    try {
      const response = await axios.post('http://98.85.246.54:8132/api/reserve/reservation', reservationData);
