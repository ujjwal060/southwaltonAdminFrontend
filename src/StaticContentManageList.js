...
const StaticContentManageList = () => {
  ...
  const handleAddStaticContent = async () => {
    try {
      const response = await axios.post('http://52.20.55.193:8132/api/StaticContentManageData/StaticContent', { policy, termsandconditions });
  ...
  const handleUpdateStaticContent = async () => {
    try {
      ...
      const response = await axios.put(`http://52.20.55.193:8132/api/StaticContentManageData/${id}`, { policy, termsandconditions });
